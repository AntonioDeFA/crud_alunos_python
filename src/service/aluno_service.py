...
class AlunoService:
    ...
    def criar_aluno(self, nome, sobrenome):
        self.log.info(f"Processo de criação de aluno inciada com nome {nome}, sobrenome {sobrenome}")
    ...
    def atualizar_aluno(self, matricula, aluno_dict):
        self.log.info(f"Processo de atualização de aluno com matricula {matricula} inciada. Valores a atualizar {aluno_dict}")

        self.validador.validar_entradas([],{"matricula":matricula,"dados alunos":aluno_dict})
    ...
    def listar_aluno_texto(self, matricula):
        ...
        if matricula:
            return self.convertedor.dict_aluno_to_string_complex_describe(self.persistencia.listar_um(matricula))        
        
        alunos_recuperados = self.persistencia.listar_todos()
        alunos = [self.convertedor.dict_aluno_to_string(aluno) for aluno in alunos_recuperados]          
        return  '\n'.join(alunos)
