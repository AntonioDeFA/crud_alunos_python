...
class AlunoService:
    ...
    def criar_aluno(self, nome, sobrenome):
        self.log.info(
            f"Processo de criação de aluno inciada "
            f"com nome {nome}, sobrenome {sobrenome}"
        )
    ...
    def atualizar_aluno(self, matricula, aluno_dict):
        self.log.info(
            f"Processo de atualização de aluno com matricula "
            f"{matricula} inciada. Valores a atualizar {aluno_dict}"
        )
    ...
    def deletar_aluno(self, matricula):
        self.log.info(f"Processo de deletar aluno "
                      f"inciado com matricula {matricula}")
    ...
    def listar_aluno_texto(self, matricula):
        self.log.info(f"Processo de listagem "
                      f"aluno inciado com matricula {matricula}")

        if matricula:
            return self.convertedor. \
                dict_aluno_to_string_complex_describe(
                 self.persistencia.listar_um(matricula))

        alunos_recuperados = self.persistencia.listar_todos()
        alunos = [
            self.convertedor.dict_aluno_to_string(aluno)
            for aluno in alunos_recuperados
        ]
        return "\n".join(alunos)

    def listar_aluno_objeto(self, matricula):
        self.log.info(f"Processo de listagem aluno "
                      f"inciado com matricula {matricula}")
