...
def test_criar_aluno_campos_preenchido_sucesso():
    ...
    aluno_service = AlunoService(
        mock.logger.return_value(""),
        mock._persistencia.return_value(""),
        mock._validador.return_value(""),
        mock._convertedor.return_value(""))
...
def test_criar_aluno_nome_vazio_excecao():
    ...
    aluno_service = AlunoService(
        mock.logger.return_value(""),
        mock._persistencia.return_value(""),
        ValidadorEntrada(mock.logger.return_value("")),
        mock._convertedor.return_value(""))
...
def test_criar_aluno_sobrenome_none_sucesso():
    ...
    assert str(error.value) == "'NoneType'" \
...
def test_atualizar_aluno_matricula_none_excecao():
    ...
    with pytest.raises(Exception) as error:
        aluno_service.atualizar_aluno(None,
                                      payload_valores_dict)
